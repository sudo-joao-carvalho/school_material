...
int main(int argc, char *argv[])
{
	...
	if (argc == 3)
	{
        ...
        // Se o numero de execucoes do processo n�o for colocado nos argumentos de entrada, define-o com um valor por defeito
        if (argc == 2)
        {
            runs = DEFAULT_RUNS;
            strcpy(nome_fich, argv[1]);
	...
	{
		printf("Repeticao %d\n",r+1);
        // Gera  o da populacao inicial
		pop = init_pop(EA_param);
        // Avalia a populacao inicial
		evaluate(pop, EA_param, mat);
		gen_actual = 1;
		// Como ainda n�o existe, escolhe-se como melhor solucao a primeira da populacao (poderia ser outra qualquer)
		best_run = pop[0];
        // Encontra-se a melhor solucao dentro de toda a populacao
		best_run = get_best(pop, EA_param, best_run);
        // Reserva  para os pais da populacao seguinte
